...
public class Maze {
  ...
  private static final int VERTICALWALL = 0;
  private static final int HORIZONTALWALL = 1;
  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
    int numElement = horiz*vert;
    DisjointSets set = new DisjointSets(numElement);

    // create a array of walls, horize wall number range from 0 to ((vert-1)*horize)-1;
    // vert wall number range from (vert-1)*horize + (horiz-1)*vert -1
    // walls[0] indicates whether this is a horize wall or not
    // if walls[0] = 1, this is a horize wall
    // else ,this is a vert wall
    int walls_size = (horiz-1)*vert + (vert-1)*horiz;
    ArrayList<int[]> walls = new ArrayList<int[]>();

    for(int h=0; h<horiz; ++h){
      for(int v=0; v<vert-1; ++v){
        int[] element = new int[3];
        element[0] = HORIZONTALWALL;
        element[1] = h;
        element[2] = v;
    ...
    for(int m=0; m<horiz-1;++m){
      for(int n=0; n<vert; ++n){
        int[] element = new int[3];
        element[0] = VERTICALWALL;
        element[1] = m;
        element[2] = n;
    ...
    for(int w=walls_size; w>1; --w){
      int random_num = randInt(w);
      int[] temp = walls.get(w-1);
      walls.set( w-1, walls.get(random_num));
      walls.set( random_num, temp);
    }
   ...
   */
  public void printArray(ArrayList<int[]> walls){

    Iterator<int[]> it = walls.iterator();
    System.out.println("walls after rearrange order is: ");
    while(it.hasNext()){
